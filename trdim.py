...
def get_m_link(link):
    ...
    for a in links:
        if a.text.strip().lower() == 'dimensions':
...
m_json_append_sql = "INSERT INTO `model_dimensions_json` VALUES(?,?) ON DUPLICATE KEY UPDATE `DimensionsJSON`=?"
...
rows = cur.fetchall()
for col in rows:
    col['Link'] = "https://www.tractordata.com/farm-tractors/010/6/2/10629-john-deere-7r-270.html"
